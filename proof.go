...
func (l *Log[D]) InclusionProof(index uint64) (*InclusionProof[D], error) {
	...
	// index < size, and size = uint64(len(l.leaves)).
	// Since len(l.leaves) fits in int, index does too.
...
func (l *Log[D]) ConsistencyProof(oldSize uint64) (*ConsistencyProof[D], error) {
	...
	// oldSize < size, and size = uint64(len(l.leaves)).
	// Since len(l.leaves) fits in int, oldSize does too.
